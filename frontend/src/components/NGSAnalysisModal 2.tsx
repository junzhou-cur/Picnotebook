...
export function NGSAnalysisModal({ isOpen, onClose }: NGSAnalysisModalProps) {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      // Step 2: Run CRISPResso if requested
      if (baseEditorOutput) {
        ...
        const crispressoResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5005'}/api/ngs/run_crispresso`, {
          ...
          body: JSON.stringify({
            gene_folder: analysisData.output_folder
          })
        });

        if (crispressoResponse.ok) {
          const crispressoData = await crispressoResponse.json();
          setAnalysisProgress(90);
          setAnalysisLog(prev => [...prev, 'CRISPResso analysis completed!']);
          setAnalysisLog(prev => [...prev, `Results in: ${crispressoData.output_folder}`]);
        } else {
          setAnalysisLog(prev => [...prev, 'CRISPResso analysis failed (manual run required)']);
        }
      }
      ...
      setAnalysisLog(prev => [...prev, `Error: ${error}`]);
      setAnalysisResults({
        success: false,
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                      ...
                      >
                        ...
                        <span className="text-sm text-gray-600">
                          Upload Excel file with sample names
                        </span>
                      </label>
                    </div>
